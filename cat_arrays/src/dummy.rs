...
impl<T> ArrayLikeMut for DummyArray<T> {
  ...
  fn flat_mut(&mut self) -> Self::FlatMut { unimplemented!() }
  ...
  fn reverse_mut(&mut self, ax1 : usize) -> Self::ReverseMut { unimplemented!() }
...
impl<T> IntoFlat for DummyArray<T> {
  ...
  fn into_flat(self) -> Self::To { unimplemented!() }
}
...
impl<T> IntoFixate for DummyArray<T> {
  type To = DummyArray<T>;
  fn into_fixate(self, ax : usize, at : isize) -> Self::To { unimplemented!() }
}

impl<T> IntoTranspose for DummyArray<T> {
  type To = DummyArray<T>;
  fn into_transpose(self, ax1 : usize, ax2 : usize) -> Self::To { unimplemented!() }
}

impl<T> IntoReverse for DummyArray<T> {
  type To = DummyArray<T>;
  fn into_reverse(self, ax : usize) -> Self::To { unimplemented!() }
}

impl<T> IntoDiagonal for DummyArray<T> {
  type To = DummyArray<T>;
  fn into_diagonal(self, ax1 : usize, ax2 : usize) -> Self::To { unimplemented!() }
}

impl<T> IntoTile for DummyArray<T> {
  type To = DummyArray<T>;
  fn into_tile(self, len : usize) -> Self::To { unimplemented!() }
}
