...
pub struct GenReshapeMutRef<'a, T, A: 'a>(&'a mut A, Vec<usize>, usize)
    where A: ArrayLike<Entry = T>;

impl<T: Clone, A: Clone> GenReshape<T, A>
    where A: ArrayLike<Entry = T>
{
...
impl<'a, T, A: 'a> GenReshapeRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
...
impl<'a, T, A: 'a> GenReshapeMutRef<'a, T, A>
...
impl<T: Clone, A> ArrayLike for GenReshape<T, A>
    where A: ArrayLike<Entry = T> + Clone
{
    type Entry = T;
...
impl<T: Clone, A> IntoFlat for GenReshape<T, A>
    where A: ArrayLike<Entry = T> + Clone
{
    type To = Self;
...
impl<T: Clone, A> IntoReshape for GenReshape<T, A>
    where A: ArrayLike<Entry = T> + Clone
{
    type To = Self;
...
impl<'a, T, A: 'a> Clone for GenReshapeRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
    fn clone(&self) -> Self {
...
#[allow(unused_variables)]
impl<'a, T, A: 'a> ArrayLike for GenReshapeRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
    type Entry = T;
    type Shape = Vec<usize>;
...
impl<'a, T, A: 'a> IntoFlat for GenReshapeRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
    type To = Self;
    fn into_flat(mut self) -> Self::To {
...
impl<'a, T, A: 'a> IntoReshape for GenReshapeRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
    type To = Self;
    fn into_reshape<I>(mut self, shape: &I) -> Self::To
...
#[allow(unused_variables)]
impl<'a, T, A: 'a> ArrayLike for GenReshapeMutRef<'a, T, A>
    where A: ArrayLike<Entry = T>
{
    type Entry = T;
    type Shape = Vec<usize>;
...
#[allow(unused_variables)]
impl<'a, T, A: 'a> ArrayLikeMut for GenReshapeMutRef<'a, T, A>
    where A: ArrayLike<Entry = T> + ArrayLikeMut
{
    ...
    fn flat_mut(&mut self) -> Self::FlatMut {
        GenReshapeMutRef(unsafe { transmute(&mut *self.0) },
                         vec![self.len()],
                         self.len())
    }
        ...
        where I: ArrayLike<Entry = usize>
    {
        ...
        GenReshapeMutRef(unsafe { transmute(&mut *self.0) },
                         FlatIter::new(shape).map(|a| *a).collect(),
                         self.len())
    }
