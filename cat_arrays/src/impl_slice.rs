...
impl<A, T> ArrayLike for A where A : Clone + Deref<Target = [T]> {
  type Entry = T;
  ...
  fn shape(&self) -> &Self::Shape { unsafe { transmute(&self.len()) } }
  ...
  fn flat(&self) -> Self::Flat { (*self).clone() }
...
#[allow(unused_variables)]
impl<A, T> ArrayLikeMut for A where A : Clone + DerefMut<Target = [T]> {
