  ...
  \begin{itemize}
    \item For every object \(o\)
      there is an arrow \(\flat\) from it, to the single-element sequence
                  that is the product of its elements \(\flat : o \to \left[ \prod o \right]\)
    \item For every object \(o\) which is a sequence of \(k\) elements,
    ...
    \item For every pair of objects \(o, o'\), which have the same product
      \(\prod o = \prod o'\), an isomorphic arrow \(\varrho\) connects them
      \( \varrho : o \to o'\), \(\varrho^{-1} : o' \to o\).
  \end{itemize}
